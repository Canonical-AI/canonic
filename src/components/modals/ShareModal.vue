        ...
        <div class="modal">
            ...
            <div v-if="!store.shareInfo">
                <div class="info-box">
                    ...
                    <span>
                        No cloud, no relay. Your doc is served straight from
                        this machine on your local network — like a local web
                        app. Stop sharing any time.
                    </span>
                </div>
            ...
            <div v-else class="share-result">
                <div class="link-row">
                    ...
                    <div class="link-box">
                        <span class="link-text">{{
                            store.shareInfo.localUrl
                        }}</span>
                        <button
                            class="copy-btn"
                            :class="copied && 'copied'"
                            @click="copy(store.shareInfo.localUrl)"
                        >
                            {{ copied ? "✓ Copied" : "Copy" }}
                        </button>
                    </div>
                    <p class="link-hint">
                        Anyone on your network can open this in a browser or in
                        Canonic.
                    </p>
                </div>
                ...
                <div class="share-status">
                    ...
                    Serving on port {{ store.shareInfo.port }} · auto-stops if
                    traffic spikes
                </div>

                <div class="modal-actions" style="margin-top: 16px">
                    <button class="btn-danger" @click="stopShare">
                        Stop sharing
                    </button>
                    <button class="btn-ghost" @click="openInBrowser">
                        Open in browser
                    </button>
                    <button class="btn-ghost" @click="$emit('close')">
                        Done
...
const permissionOptions = [
    {
        ...
        label: "View",
        icon: Eye,
        hint: "Others can read the document.",
    },
    {
        value: "comment",
        label: "Comment",
...
const activePermissionIcon = computed(
    () =>
        permissionOptions.find((o) => o.value === store.shareInfo?.permission)
            ?.icon ?? Eye,
);
